...
def create_evaluation_dataset():
    return [
        {
            "query": "¿Cuál es el protocolo de atención en caso de urgencia médica?",
            "expected_context": "protocolo de atención de urgencias",
            "ground_truth": "Ante síntomas graves como dolor intenso, dificultad para respirar o pérdida de conciencia, diríjase inmediatamente a la sala de urgencias ubicada en el primer piso. El personal médico está disponible las 24 horas."
        },
        {
            "query": "¿Qué debo hacer si tengo fiebre y dolor de cabeza?",
            "expected_context": "guía de síntomas comunes",
            "ground_truth": "Para fiebre, tos, dolor de cabeza o malestar general, puede solicitar una consulta médica presencial o virtual. Se recomienda no automedicarse y registrar sus síntomas en la plataforma de pacientes."
        },
        {
            "query": "¿Cuáles son los horarios de atención de la clínica?",
            "expected_context": "horarios de atención",
            "ground_truth": "La clínica Salud Integral atiende de lunes a viernes de 8:00 a 20:00 horas y sábados de 9:00 a 14:00 horas. Para agendar una cita, comuníquese al teléfono 2222-3333 o utilice el portal web."
        },
        {
            "query": "¿Cómo puedo solicitar mis exámenes médicos?",
            "expected_context": "preguntas frecuentes",
            "ground_truth": "Puede solicitarlos en recepción o a través del portal de pacientes."
        },
        {
            "query": "¿Qué hago si pierdo mi receta médica?",
            "expected_context": "preguntas frecuentes",
            "ground_truth": "Solicite una nueva en la consulta médica o por correo electrónico."
        },
        {
            "query": "¿Cómo retiro los medicamentos recetados?",
            "expected_context": "procedimiento para entrega de medicamentos",
            "ground_truth": "Los medicamentos recetados se retiran en la farmacia de la clínica presentando su cédula y receta médica. Horario de farmacia: lunes a viernes de 8:00 a 19:00 horas."
        },
        {
            "query": "¿Cómo se protegen mis datos personales en la clínica?",
            "expected_context": "política de privacidad",
            "ground_truth": "Todos los datos personales y médicos son tratados bajo estricta confidencialidad según la Ley de Protección de Datos. Para más información, consulte el documento de privacidad en la web."
        },
        {
            "query": "¿Qué hago en caso de emergencia médica fuera de horario?",
            "expected_context": "contacto y emergencias",
            "ground_truth": "Para emergencias médicas fuera de horario, llame al 911 o acuda al hospital más cercano. Para dudas administrativas, escriba a contacto@saludintegral.cl."
        }
    ]
...
def main():
    st.markdown(
        ...
        <style>
        .stApp {
            background-color: #04313B !important;
        }
        body {
            background-color: #04313B !important;
        }
    ...
    st.title("Clinica VitalCare")
    st.write("Atención médica con calidez y tecnología.")
    ...
    if "eval_rag" not in st.session_state:
        st.session_state.eval_rag = {
            'documents': [
                "Protocolo de atención de urgencias: Ante síntomas graves como dolor intenso, dificultad para respirar o pérdida de conciencia, diríjase inmediatamente a la sala de urgencias ubicada en el primer piso. El personal médico está disponible las 24 horas.",
                "Guía de síntomas comunes: Para fiebre, tos, dolor de cabeza o malestar general, puede solicitar una consulta médica presencial o virtual. Se recomienda no automedicarse y registrar sus síntomas en la plataforma de pacientes.",
                "Horarios de atención: La clínica Salud Integral atiende de lunes a viernes de 8:00 a 20:00 horas y sábados de 9:00 a 14:00 horas. Para agendar una cita, comuníquese al teléfono 2222-3333 o utilice el portal web.",
                "Preguntas frecuentes: ¿Cómo solicito mis exámenes médicos? Puede solicitarlos en recepción o a través del portal de pacientes. ¿Qué hago si pierdo mi receta? Solicite una nueva en la consulta médica o por correo electrónico.",
                "Procedimiento para entrega de medicamentos: Los medicamentos recetados se retiran en la farmacia de la clínica presentando su cédula y receta médica. Horario de farmacia: lunes a viernes de 8:00 a 19:00 horas.",
                "Política de privacidad: Todos los datos personales y médicos son tratados bajo estricta confidencialidad según la Ley de Protección de Datos. Para más información, consulte el documento de privacidad en la web.",
                "Contacto y emergencias: Para emergencias médicas fuera de horario, llame al 911 o acuda al hospital más cercano. Para dudas administrativas, escriba a contacto@saludintegral.cl."
            ],
            'embeddings': None,
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["❓ Consulta", "📚 Documentos", "📊 Métricas", "🧪 Evaluación", "📈 Analytics", "📱 Chatbot"])
    ...
    with tab1:
        ...
        with col1:
            ...
            with col_a:
                top_k = st.slider("Documentos a evaluar:", 1, 8, 3)
            with col_b:
                ...
                if st.session_state.eval_rag['documents'] and st.session_state.eval_rag['embeddings_model']:
                    with st.spinner("Generando embeddings con LangChain..."):
                        ...
                        if embeddings is not None:
                            ...
                            st.success("✅ Embeddings generados correctamente con LangChain")
                        else:
        ...
        if st.button("🚀 Consultar con Métricas") and query:
            if st.session_state.eval_rag['embeddings'] is None:
                ...
                with st.spinner("Procesando con métricas..."):
                    ...
                    st.subheader("📑 Documentos Evaluados para la respuesta")
                    ...
                    with col2:
                        st.metric("Evaluación de Documentos", f"{metrics['retrieval_time']:.2f}s")
                    with col3:
                        st.metric("Generación de Respuesta", f"{metrics['generation_time']:.2f}s")
                    with col4:
                        st.metric("Documentos evaluados", metrics['docs_retrieved'])
    ...
    with tab2:
        st.header("📄 Todos tus Documentos")
        ...
        with col1:
            ...
            for i, doc in enumerate(st.session_state.eval_rag['documents']):
                with st.expander(f"Documento {i+1} ({len(doc)} caracteres)"):
                    ...
                    with col_edit:
                        if st.button(f"✏️ Editar contenido", key=f"edit_{i}"):
                            st.session_state[f'editing_doc_{i}'] = True
                    
                    with col_delete:
                        if st.button(f"🗑️ Eliminar documento", key=f"delete_{i}"):
        ...
        with col2:
            ...
            if st.button("📝 Agregar Documento"):
                if new_doc.strip():
                    ...
                    # Reset embeddings when documents change
                    st.session_state.eval_rag['embeddings'] = None
                    st.success("Documento agregado con exito")
                    st.rerun()
                else:
    ...
    with tab6:  # Define la pestaña `tab6`
        st.title("💬 VitalBot: Tu asistente de salud")
        st.caption("🚀 A Streamlit chatbot powered by OpenAI")

        # Slider para ajustar la temperatura
        temperature = st.slider("Temperatura de la respuesta", min_value=0.0, max_value=2.0, value=0.7, step=0.05)

        # Obtenemos la clave API desde las variables de entorno
        openai_api_key = os.getenv("OPENAI_API_KEY")
        if not openai_api_key:
            st.error("⚠️ La clave API de OpenAI no está configurada como una variable de entorno.")
            st.stop()

        # Inicializamos el estado de sesión para guardar los mensajes del chat
        if "messages" not in st.session_state:
            st.session_state["messages"] = [
                {"role": "assistant", "content": "¡Hola! En clinica VitalCare estamos para ayudarte. ¿Tienes alguna pregunta sobre tu salud o nuestros servicios?"}
            ]
        ...
        if user_input:
            ...
            try:
                ...
                st.error(f"❌ Ocurrió un error con la API de OpenAI: {e}")
